...
async function joinLeaderTuan() {
  let res = await updateTuanIdsCDN(), res2 = await updateTuanIdsCDN("https://raw.githubusercontent.com/linghuaqiandao/myactions/main/test/jd_updateFactoryTuanId.json")
  if (!res) res = await updateTuanIdsCDN('https://raw.githubusercontent.com/linghuaqiandao/myactions/main/test/jd_updateFactoryTuanId.json');
...
function updateTuanIdsCDN(url = 'https://raw.githubusercontent.com/linghuaqiandao/myactions/main/test/jd_updateFactoryTuanId.json') {
  return new Promise(async resolve => {
    const options = {
...
function readShareCode() {
  console.log(`开始`)
  return new Promise(async resolve => {
    $.get({url: "https://raw.githubusercontent.com/linghuaqiandao/myactions/main/test/JD_Dream_Factory.json",headers:{
      "User-Agent": "Mozilla/5.0 (iPhone; CPU iPhone OS 13_2_3 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/13.0.3 Mobile/15E148 Safari/604.1 Edg/87.0.4280.88"
    }}, async (err, resp, data) => {
      try {
        if (err) {
  ...
  return new Promise(async resolve => {
    ...
    if (!tuanActiveId) {
      ...
      if ($.tuanConfigs && $.tuanConfigs['tuanActiveId']) {
        ...
        if (!$.tuanConfigs) {
          await updateTuanIdsCDN('https://raw.githubusercontent.com/linghuaqiandao/myactions/main/test/jd_updateFactoryTuanId.json');
          if ($.tuanConfigs && $.tuanConfigs['tuanActiveId']) {
            tuanActiveId = $.tuanConfigs['tuanActiveId'];
            console.log(`拼团活动ID: 获取成功 ${tuanActiveId}\n`)
          } else {
