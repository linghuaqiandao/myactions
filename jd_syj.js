...
!(async () => {
  ...
  await getAuthorShareCode('https://raw.githubusercontent.com/linghuaqiandao/myactions/main/test/jd_syj.json');
  await getAuthorShareCode('https://raw.githubusercontent.com/linghuaqiandao/myactions/main/test/jd_syj.json');
  await getRandomCode();
...
async function getRandomCode() {
  await $.http.get({url: `https://raw.githubusercontent.com/linghuaqiandao/myactions/main/test/jd_syj.json`, timeout: 10000}).then(async (resp) => {
    if (resp.statusCode === 200) {
      try {
        let { body } = resp;
        body = JSON.parse(body);
