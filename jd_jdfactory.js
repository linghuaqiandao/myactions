...
function readShareCode() {
  console.log(`开始`)
  return new Promise(async resolve => {
    $.get({url: "https://raw.githubusercontent.com/linghuaqiandao/myactions/main/test/JD_Factory.json",headers:{
      ...
      try {
        if (err) {
          ...
          $.get({url: `https://raw.githubusercontent.com/chmkk01/myactions/main/test/JD_Factory.json`, 'timeout': 10000},(err, resp, data)=>{
          data = JSON.parse(data);})
        } else {
          if (data) {
            console.log(`随机取助力码放到您固定的互助码后面(不影响已有固定互助)`)
