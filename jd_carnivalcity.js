...
function updateShareCodesCDN(url = 'https://raw.githubusercontent.com/linghuaqiandao/myactions/main/test/carnivalcity.json') {
  return new Promise(resolve => {
...
function readShareCode() {
  ...
  return new Promise(async resolve => {
    $.get({url: `https://raw.githubusercontent.com/linghuaqiandao/myactions/main/test/carnivalcity.json`, 'timeout': 20000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          console.log(`${$.name} API请求失败，请检查网路重试`)
